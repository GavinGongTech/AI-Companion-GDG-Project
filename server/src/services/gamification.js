...
export async function getGamificationData(uid) {
  try {
    const userRef = db.collection('users').doc(uid)
    const gamSnap = await userRef.collection('gamification').doc('stats').get()
    const gam = gamSnap.exists ? gamSnap.data() : {}
    ...
    const unlocked = new Set(gam.unlockedAchievements || [])
    const achievementData = { streak, conceptCount, maxAccuracy, quizCount: gam.quizCount || 0 }
    const newlyUnlocked = []
    ...
    logger.error({ err, uid }, 'getGamificationData failed')
    return {
...
export async function addXP(uid, points, reason) {
  try {
    ...
    logger.warn({ err, uid, reason }, 'addXP failed (non-critical)')
  }
}
...
export async function updateStreak(uid) {
  try {
    ...
    logger.warn({ err, uid }, 'updateStreak failed (non-critical)')
  }
}
